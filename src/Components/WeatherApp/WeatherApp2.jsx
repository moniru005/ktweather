...
const WeatherApp2 = () => {
  ...
  const handleSearch = async () => {
      try {
        const res = await axiosPublic.get(`/weather/${city}`);
        setWeather(res.data);
      } catch (error) {
    ...
    useEffect(() => {
      const fetchData = async () =>{
        try {
          const res = await axiosPublic.get(`/weather/Dhaka`);
          setWeather(res.data);
        } catch (error) {
          console.error("Error fetching weather data:", error);
        }
      ...
      <div className="flex flex-col justify-center items-center py-4">
        <div className="flex flex-row items-center pb-4">
          <input
            ...
            className="p-4 rounded-l-lg border border-gray-900 capitalize"
          />
          <button 
        ...
        <div className="mb-6 flex flex-col items-center space-y-3">
          <h2 className="py-4 font-medium text-stale-900 text-center">
            <span className="text-4xl font-bold">{weather.location?.name}</span>
          </h2>
        </div>
      </div>
